...
test("ensureSchema creates settings table (key-value)", () => {
  ...
  db.prepare("INSERT INTO settings (key, value) VALUES ('node_timeout_default_ms', '600000')").run();
...
test("ensureSchema creates idx_workflow_runs_wf_queued index", () => {
  const db = makeDb();
  ensureSchema(db);
  const idxs = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='workflow_runs'").all().map((r) => r.name);
  assert.ok(
...
test("markInflightRunsInterrupted sets queued/running rows to terminated with report.reason", () => {
  ...
  const rows = db.prepare("SELECT id, status, ended_at, report FROM workflow_runs ORDER BY id").all();
