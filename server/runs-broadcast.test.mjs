...
function parseSSE(chunk) {
  // SSE frames may contain id/event lines before the data line.
  const frames = chunk.split('\n\n').filter(Boolean);
  const last = frames[frames.length - 1];
  const dataLine = last.split('\n').find((line) => line.startsWith('data:'));
  if (!dataLine) return null; // heartbeat
...
test('onTerminal broadcasts run_terminal with full report + schema_snapshot after DB write', async () => {
  ...
  const schemaParsed =
    typeof ev.schema_snapshot === 'string' ? JSON.parse(ev.schema_snapshot) : ev.schema_snapshot;
...
test('run_terminal broadcast goes only to subscribers of that workflow', async () => {
  ...
  const queue = createRunQueue({
    ...
    onTerminal: createCapturingOnTerminal(
      db,
      async () => ({ workflowStatus: { success: true } }),
      bus,
    ),
    onEvent: (w, e) => bus.broadcast(w, e),
  });
