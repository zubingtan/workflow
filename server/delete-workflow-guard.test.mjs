 ...
 *   - On a successful delete, bus.broadcast is called once with
...
function makeApp({ broadcast } = {}) {
  const { db, dir } = setupDb();
  const eventBus = createRunsEventBus();
  if (broadcast) {
    eventBus.broadcast = broadcast;
  }
...
test('DELETE /workflows/:id returns 409 with activeCount when a queued run exists', async () => {
  ...
  const res = await app.fetch(new Request('http://localhost/workflows/wf_1', { method: 'DELETE' }));
...
test('DELETE /workflows/:id returns 409 while queued; succeeds after the run becomes terminated', async () => {
  ...
  db.prepare("UPDATE workflow_runs SET status='terminated' WHERE id=?").run('run_q1');
...
test('DELETE /workflows/:id broadcasts workflow_deleted via bus.broadcast on success', async () => {
  const calls = [];
  const { app } = makeApp({
    broadcast: (workflowId, event) => calls.push({ workflowId, event }),
  });
  ...
  assert.deepEqual(calls[0], {
    workflowId: 'wf_1',
    event: { type: 'workflow_deleted', workflowId: 'wf_1' },
  });
});
