...
- **Business components NOT yet migrated.** This ADR covers the token layer foundation (Ticket A). Ticket B migrates the 83 stylelint-violating files + problem-panel + sidebar/editor header to consume `--app-*` tokens. Until then, visual appearance is unchanged because token values match the pre-existing hardcoded values.
