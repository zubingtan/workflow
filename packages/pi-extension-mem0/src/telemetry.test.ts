...
beforeEach(() => {
  // Fork: telemetry defaults OFF; tests opt in explicitly.
...
describe('pi-agent telemetry', () => {
  ...
  it('respects the MEM0_TELEMETRY opt-out (fork default: OFF)', () => {
  ...
  it('sends nothing when MEM0_TELEMETRY is unset (fork default: OFF)', () => {
    delete process.env.MEM0_TELEMETRY;
    captureEvent('pi.session.start', {}, CTX);
    captureToolEvent('add', {}, CTX);
    expect(_getEventQueue()).toHaveLength(0);
