...
import type { Mem0Config, ScopeContext, Scope, MemoryClientLike } from './types.ts';
...
export function registerCommands(
  ...
  pi.registerCommand('mem0-remember', {
    ...
    handler: async (args, ctx) => {
      ...
      const result = await mem0.add([{ role: 'user', content: text }], {
        ...addParams,
    ...
    handler: async (args, ctx) => {
      ...
      const forgotten = (mem: Parameters<typeof formatMemoryCompact>[0]) => {
        ...
        sendFeedback(
          'mem0-forget',
          [
            `**Forgotten from ${config.defaultScope} memory**`,
            `- ${formatMemoryCompact(mem)}`,
          ].join('\n')
        );
      };
      ...
      const labels = memories.map((m: { id: string; memory?: string }) => formatMemoryCompact(m));
      const selected = await ctx.ui.select(
        `Found ${pluralize(
          memories.length,
          'match',
          'matches'
        )} for "${query}" — which should I delete?`,
        labels
      );
      if (!selected) {
        sendFeedback('mem0-forget', '**Cancelled** — no memories deleted.');
  ...
  pi.registerCommand('mem0-search', {
    ...
    handler: async (args, ctx) => {
      ...
      sendFeedback(
        'mem0-search',
        [
          `**${pluralize(memories.length, 'match', 'matches')} for "${query}"** · ${
            config.defaultScope
          } scope`,
          '',
  ...
  pi.registerCommand('mem0-tour', {
    ...
    handler: async (args, ctx) => {
      ...
      const validScopes: Scope[] = ['agent', 'session'];
      if (raw && !validScopes.includes(raw as Scope)) {
        ctx.ui.notify(
          `Invalid scope "${raw}". Must be one of: ${validScopes.join(', ')}`,
          'warning'
        );
        return;
      }
      ...
      if (memories.length === 0) {
        ...
        sendFeedback(
          'mem0-tour',
          `**No memories in ${scope} scope yet** — store one with \`/mem0-remember\`.`
        );
        return;
      }
  ...
  pi.registerCommand('mem0-dream', {
    description:
      'Consolidate memories — merge duplicates, prune stale entries, resolve contradictions',
    handler: async (_args, ctx) => {
      ...
      pi.sendMessage(
        { customType: 'mem0-dream', content: DREAM_PROTOCOL, display: false },
        { triggerTurn: true }
      );
      sendFeedback(
        'mem0-dream',
  ...
  pi.registerCommand('mem0-pin', {
    ...
    handler: async (args, ctx) => {
      ...
      const alreadyPinned = (mem: Parameters<typeof formatMemoryCompact>[0]) => {
        sendFeedback(
          'mem0-pin',
          ['**Already pinned**', `- ${formatMemoryCompact(mem)}`].join('\n')
        );
      };
  ...
  pi.registerCommand('mem0-scope', {
    description: 'Change default memory scope for this session (agent, session)',
    handler: async (args, ctx) => {
      const scope = args?.trim().toLowerCase();
      const valid: Scope[] = ['agent', 'session'];
      ...
      if (!scope) {
        sendFeedback(
          ...
          [
            ...
            `New memories save to the **${
              config.defaultScope
            }** pool. Switch with \`/mem0-scope <${valid.join(' | ')}>\`.`,
          ].join('\n')
        );
        return;
      }
