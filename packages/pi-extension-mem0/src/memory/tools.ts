...
import type { MemoryClientLike } from '../types.ts';
  ...
  return async (params: ToolParams, signal?: AbortSignal) => {
    ...
    switch (params.action) {
      ...
      case 'add': {
        ...
        const result = await mem0.add([{ role: 'user', content: params.content }], {
          ...addParams,
...
export function registerMemoryTool(
  ...
  pi.registerTool({
    ...
    promptGuidelines: [
      ...
      'Always use the default agent scope unless the user EXPLICITLY asks to search only this run — only then use scope "session"',
    ...
    parameters: Type.Object({
      ...
      scope: Type.Optional(
        StringEnum(['agent', 'session'] as const, {
          description:
            'Where to read/write: "agent" (default — every memory of this agent, shared across all workflows that use it), or "session" (this run only). Omit for normal queries.',
        })
      ),
    }),
