...
describe('registerCommands', () => {
  ...
  beforeEach(() => {
    pi = makePi();
    mem0 = makeMem0();
    defaultConfig.defaultScope = 'agent';
  ...
  describe('/mem0-pin', () => {
    ...
    it('skips already-pinned memories with a visible message', async () => {
      ...
      expect(pi.sendMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          content: expect.stringContaining('Already pinned'),
          display: true,
        })
      );
    });
  ...
  describe('/mem0-search', () => {
    it('performs server-side semantic search with a relevance threshold', async () => {
      ...
      expect(mem0.search).toHaveBeenCalledWith(
        'my preferences',
        // D5: cloud-only `rerank` is removed from the self-hosted client.
  ...
  describe('/mem0-scope', () => {
    ...
    it('sends a visible confirmation after changing scope', async () => {
      const ctx = makeCtx();
      await pi._invoke('mem0-scope', 'session', ctx);
      expect(pi.sendMessage).toHaveBeenCalledWith(
        expect.objectContaining({
          customType: 'mem0-scope',
          content: expect.stringContaining('Scope changed to session'),
          display: true,
        })
      );
    });
