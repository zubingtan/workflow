...
pnpm install
cp .env.example .env
pnpm dev            # http://localhost:4001 (fake-provider :4010 + dev server :4001)
```

Dev mode runs on a single port `:4001` (Hono API + rsbuild HMR SPA via
`middlewareMode`) with data in `~/.config/workflow-dev/`. Prod runs on `:4000`
with data in `~/.config/workflow/`. E2E uses isolated ports `:4099` + `:4011`.

### One-time data migration (existing users)

If you previously used `~/.config/workflow/` for dev (before dev/prod data
dirs were split), copy it once to preserve your data:

```bash
cp -r ~/.config/workflow/ ~/.config/workflow-dev/
```

Fresh clones don't need this — both dirs start empty.
...
export const StartNodeRegistry: FlowNodeRegistry = {
  ...
  info: {
    icon: iconStart,
    description:
      'The starting node of the workflow, used to set up information needed to launch the workflow.',
  },
  formMeta, // Form configuration
  canAdd() {
    return false;
  }, // Disallow multiple start nodes
};
```
